...
nod* copiazaDinHeapInArbore(Heap &h) {
	if (h.dimensiune > 0)
	{
		...
		{
			Factura f = creareFactura(h.vector[i].nrZilePanaLaScadenta, h.vector[i].idFactura, h.vector[i].numeFactura, h.vector[i].furnizorFactura, h.vector[i].sumaPlata);
			if (h.vector[i].nrZilePanaLaScadenta < 10) {
				rad = inserareInArbore(rad, f);
		}
...
nod* stergereABC(nod* &rad)
{
	if (rad)
	{
		stergereABC(rad->stanga);
		stergereABC(rad->dreapta);
		free(rad->info.numeFactura);
		free(rad->info.furnizorFactura);
		free(rad);
		rad = NULL;
		return rad;
	}
